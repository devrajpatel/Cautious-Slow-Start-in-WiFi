...
TcpCongestionOps::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpCongestionOps")
    ...
    .AddAttribute ("RTT",
                   ...
                   MakeTimeAccessor (&TcpCongestionOps::m_curRtt),
                   MakeTimeChecker ())                                    //bhanu
  ;
  return tid;
}
...
uint32_t
TcpNewReno::CSlowStart (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  NS_LOG_FUNCTION (this << tcb << segmentsAcked);
  double delay;
  if (segmentsAcked >= 1)
    {
        if(tcb->m_cWnd==tcb->m_initialCWnd){
		tcb->prev_rtt_=MilliSeconds (0);           
                tcb->cur_rtt_=MilliSeconds (0);
                tcb->n=0; 
	}
	if(tcb->n!=0)
		tcb->prev_rtt_=(tcb->cur_rtt_+ tcb->prev_rtt_* tcb->n)/(tcb->n +1);
	tcb->cur_rtt_=m_curRtt;
       // std::cout << "Current Rtt is" << m_curRtt << EOF;
	tcb->n = tcb->n+1;
	if((tcb->cur_rtt_ < tcb->prev_rtt_) || tcb->prev_rtt_== 0){           
		tcb->m_cWnd += tcb->m_segmentSize; 
	}                           
	else{                                         
		delay = (double)(tcb->cur_rtt_.GetMilliSeconds () - tcb->prev_rtt_.GetMilliSeconds ()); 
        	if(delay==0 || (tcb->prev_rtt_/delay)>=(tcb->m_cWnd/2.0)) 
        		tcb->m_cWnd += 0.5*tcb->m_segmentSize;                             
        	else if((tcb->prev_rtt_/delay)<1)
        		tcb->m_cWnd=tcb->m_cWnd;                
        	else if ((tcb->prev_rtt_/delay>1) && (tcb->prev_rtt_/delay <tcb->m_cWnd/2))              
        		tcb->m_cWnd=tcb->m_cWnd+((tcb->prev_rtt_.GetMilliSeconds ()/delay)/tcb->m_cWnd);
	}
      //tcb->m_cWnd += tcb->m_segmentSize;
      //NS_LOG_INFO ("In SlowStart, updated to cwnd " << tcb->m_cWnd << " ssthresh " << tcb->m_ssThresh);
      return segmentsAcked - 1;
...
TcpNewReno::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  if (tcb->m_cWnd < tcb->m_ssThresh)
    { 
      segmentsAcked = flag_CSS ? CSlowStart (tcb, segmentsAcked) : SlowStart (tcb, segmentsAcked);
    }
