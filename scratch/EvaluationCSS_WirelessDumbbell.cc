...
{ 
  ...
  central.SetDeviceAttribute ("DataRate", StringValue ("1Mbps"));
  ...
  central.EnablePcapAll("TcpNewReno_CSS",false);
 ...
 wp2p.GetLeft ()->GetDevice (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&RxDrop));
  ...
  double averageThroughput = ((cur * 8) / (1e5  * Simulator::Now ().GetSeconds ()));
