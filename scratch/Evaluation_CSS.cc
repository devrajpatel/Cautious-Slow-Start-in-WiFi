/*Exhaustive Evaluation of Cautious Slow Start in Wifi Network
Assignment #GP3 of Wireless Network (CS821)
*/
...
void CalculateThroughput ()
{
    ...
    double cur = ((sink1->GetTotalRx() - lastTotalRx1) + (sink2->GetTotalRx() - lastTotalRx2) + (sink3->GetTotalRx() - lastTotalRx3)) *(double) 8/1e5;     
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    /** TCP Congestion Ops (TcpNewReno) Protocol  **/
    ...
    */
    ...
    wirtop.GetLeft ()->GetDevice (0) -> SetAttribute ("ReceiveErrorModel", PointerValue (em));
//    wirtop.GetLeft ()->GetDevice (1) -> SetAttribute ("ReceiveErrorModel", PointerValue (em));
 //   wirtop.GetLeft ()->GetDevice (2) -> SetAttribute ("ReceiveErrorModel", PointerValue (em));
    ...
    */
     ...
     */
    AddressValue remoteAddress2 (InetSocketAddress (wirtop.GetRightIpv4Address (1), port));
     ...
     */
    AddressValue remoteAddress3 (InetSocketAddress (wirtop.GetRightIpv4Address (2), port));
    ...
    ApplicationContainer senderApp3 = tcpSender3.Install (wirtop.GetLeft (2));
    ...
    */
    bottleneckLink.EnablePcapAll ("CSS_Evaluate", false);
    ...
    */
	...
	//Average Throughput
	double averageThroughput = ((totalRxBytesCounter * 8) / (1e6  * Simulator::Now ().GetSeconds ()));
	std::cout << "\nAverage throughtput: " << averageThroughput << " Mbit/s" << std::endl;

	//Calculation of Goodput
    NS_LOG_UNCOND ("----------------------------\n:::::::::"
                     << "\nGoodput Bytes/sec:"
